...
api.changeNickname(`${(!global.config.BOTNAME) ? "Made by Sam" : global.config.BOTNAME}`, handleReply.pending[singleIndex - 1].threadID, api.getCurrentUserID());
            api.sendMessage("", event.threadID, () => api.sendMessage({body:` تمت الموافقة بنجاج\n-`, attachment: fs.createReadStream(__dirname + "/rankup/trolled.mp3")} ,handleReply.pending[singleIndex - 1].threadID));
            count+=1;
...
module.exports.run = async function({ api, event, args, permission, handleReply }) {
    ...
    case "t":
    case "المجموعات": {
   const permission = ["100005494475389","100069841597479"];
    if (!permission.includes(event.senderID)) return api.sendMessage("ماعدك صلاحية أستعمال هذا الامر  :>", event.threadID, event.messageID);
     const { threadID, messageID } = event;
    const commandName = this.config.name;
    var msg = "", index = 1;
